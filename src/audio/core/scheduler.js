import {PassThroughNode} from './audio-node';
import PriorityQueue from './priority-queue';
import Pattern from '../pattern/pattern';

/**
 * A sample-accurate scheduler built as an AudioGraphNode.  The scheduler works
 ...
 *
 */
export default class Scheduler extends PassThroughNode {
    ...
    */    
    constructor(audiolib, bpm) {
        ...
        this.lastBeatTime = 0;
        this.beatLength = 60 / this.bpm * this.audiolib.device.sampleRate;
    }
    ...
    play(patterns, durationPattern, callback) {
        ...
        event.time = this.audiolib.device.getWriteTime();
        this.queue.push(event);
        return event;
    }
     ...
     * @return {Object} The event object.
     */
    playAbsolute(beat, patterns, durationPattern,
                                                callback) {
        if (beat < this.beat ||
            beat == this.beat && this.time > this.lastBeatTime) {
            // Nah
    ...
    tickClock() {
        this.time += 1;
        this.seconds = this.time / this.audiolib.device.sampleRate;
